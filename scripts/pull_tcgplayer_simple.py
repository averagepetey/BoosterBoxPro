#!/usr/bin/env python3
"""
Simple TCGplayer data pull script - minimal dependencies
"""
import json
import os
...
# All 18 boxes with TCGplayer URLs
BOXES = {
...
def main():
    ...
    for i, (box_id, (name, url)) in enumerate(BOXES.items(), 1):
        ...
        try:
            ...
            data = items[0]
            avg_daily = float(data.get("averageDailyQuantitySold", 0) or 0)
            market_price = float(data.get("trendingMarketPrice", data.get("marketPrice", 0)) or 0)
            daily_vol = round(avg_daily * market_price, 2)
            
            # Get floor price from latest bucket
            buckets = data.get("buckets", [])
            floor = float(buckets[0].get("lowSalePrice", 0) or 0) if buckets else market_price
            
            # Update or create entry
            entry = next((e for e in entries if e.get("box_id") == box_id and e.get("date") == today), None)
            if not entry:
                entry = {"box_id": box_id, "date": today}
